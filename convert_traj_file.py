...
with open("data/trajectories.ge.45N.txt.ARCTICGRIS","r") as f:
...
icols= np.empty([int(nstorms),max(ntimes)])
...
for i in range(nstorms):
    ...
    for row in tmp:
        ...
        icols[i,j] = (int(columns[0]))
        lons[i,j] = (float(columns[1]))
        lats[i,j] = (float(columns[2]))
        ps[i,j] = (float(columns[3])/100) #convert from Pa to hPa
        wind_max[i,j] = (float(columns[4]))
...
new_file = Dataset('data/trajectories.ARCTICGRIS.ge.45N.nc','w',format='NETCDF4')
...
#create variables
cicol = new_file.createVariable('cicol','i4',('stormID','time'),fill_value = -9999)
...
cicol.units = 'index'
cicol.long_name = 'storm center column index'
